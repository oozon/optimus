...
function[Grid,Layer,Q, SpecOut] = WAKZK_bowl_test()
...
%% Transducer %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Tx.f = 5e5;		% frequency (Hz) #X
Tx.a1 = 0;		% inner radius (cm) #X
Tx.a2 = 3.2;		% outer radius (cm) #X
Tx.d = 6.4;		% geometric focus (cm) [= Inf if planar array] #X
Tx.P = 100;		% total acoustic power (W) #X

% Tx.f = 1.1e6;		% frequency (Hz)
% Tx.a1 = 0;		% inner radius (cm)
% Tx.a2 = 3.2;		% outer radius (cm)
% Tx.d = 6.32;		% geometric focus (cm) [= Inf if planar array]
% Tx.P = 50;		% total acoustic power (W)

% Tx.f = 1e6;		% frequency (Hz)
% Tx.a1 = 0;%1;		% inner radius (cm)
% Tx.a2 = 0.5;%3;		% outer radius (cm)
% Tx.d = Inf;%5;		% geometric focus (cm) [= Inf if planar array]
% Tx.P = 800;		% total acoustic power (W)
...
Grid.Z = Tx.d + 1;		% max axial distance (cm)
...
%% Spatial averaging %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
hd = 0.0;		% diameter of hydrophone element (mm).  [hd = 0 no averaging] 
...
Layer(1).beta = 3.5;	% nonlinear parameter #X ESTE HACE CEROS
...
% Layer(1).z = 0;		% material transition distance (cm)	
% Layer(1).c = 1640;	% small-signal sound speed (m/s)
% Layer(1).rho = 1070;	% mass density (kg/m^3)
% Layer(1).alpha = 7.0;	% attenuation at 1MHz (dB/m)
% Layer(1).fraction = 0;	% fraction of attenuation due to absorption	
% Layer(1).eta = 1.3;	% exponent in attenuation power law
% Layer(1).beta = 1+7.48/2;	% nonlinear parameter
...
w = 1.1*a2p;		% width (radius) of physical domain (Tx radius + 5% pad)
